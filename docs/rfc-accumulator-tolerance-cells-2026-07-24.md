...
the `(compute, acc)` classification).
**Clauses BOUND/REUSED (no text change):** KISS-Classify §6.7-0006 (`<acc>` key
coordinate, C2), §6.6-0003 (extent-free key, C4), §6.8-0004 (tolerance lives in the
Contract Guarantees, C4).
...
> §6.5-0007 wide-precision oracle; every narrower-`A` cell's reference is a defined,
> representable point whose distance from that oracle is the cell's **characteristic**
...
removing the self-contradiction: the widest-`A` cell still binds against wide truth
(§6.5-0007), and narrower cells bind against their own fully-specified deterministic
reference (order and both roundings pinned). Wide-truth conformance is thus preserved
**transitively**, not discarded.
